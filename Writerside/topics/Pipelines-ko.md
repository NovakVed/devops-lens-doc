...
로그를 확인하는 동안에도 **Summary** 탭은 계속 표시됩니다. 작업이나 스테이지를 열면 **Logs** 탭이 추가되며 작업 탐색기는 Pipelines 도구 창에 유지됩니다. 보고서 탭은 실행이 끝나고 해당 데이터가 있을 때 나타납니다. 순서는 **Summary, Logs, Tests, Extensions, Environments, Code coverage**이며, 사용할 수 없는 탭은 생략됩니다.

| 탭                | 표시되는 조건                           | 표시 내용                                                                                                                                           |
|-------------------|-----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
| **Summary**       | 항상 표시                               | 스테이지 그래프, 그리고 실행에 리포지토리 리소스가 있을 때는 **Repositories** 카드(열 **Resource Name, Repository, Branch/Tag, Version, Related**). |
| **Logs** | 작업이나 스테이지를 연 뒤 | 선택한 작업의 단계 로그 또는 스테이지 정보. 탭을 전환해도 표시 위치를 유지합니다. |
| **Tests**         | 종료된 실행이 테스트 결과를 보고했을 때 | 간결한 결과별 개수와 정렬 가능한 네이티브 테스트 결과 테이블(아래 참조). |
| **Extensions**    | 종료된 실행이 확장 요약을 게시했을 때   | 확장이 게시한 Markdown 또는 HTML 보고서(예: SonarQube 품질 게이트). 은은한 구분선으로 나누고 링크를 유지합니다. |
| **Environments**  | 종료된 실행이 환경에 배포했을 때        | **Environment, Last stage, Result, Finished** 테이블 - 프로젝트 전체의 환경별 최신 보기가 아니라 **이 실행의 배포만** 나열합니다.                   |
| **Code coverage** | 종료된 실행이 커버리지를 게시했을 때    | **Metric, Covered, Total, Coverage** 열이 있는 정렬 가능한 네이티브 테이블. 합계가 양수가 아닌 지표의 백분율은 대시로 표시합니다. |
...
**Total tests**, **Passed**, **Failed**, **Others** 개수를 간결하게 표시하며, 가능한 경우 **Pass percentage**와 **Run duration**도 표시합니다. 필터 막대는 결과 테이블 바로 위에 있습니다:
...
- 패싯 칩 - **Test file**과 **Owner**(각각 서로 다른 값이 둘 이상일 때만 표시), 그리고 **Outcome** 칩. Outcome 버킷은 **Failed, Aborted, Passed,
  Not Impacted, Others**이며, 기본 필터는 해당 결과가 있으면 **Failed + Aborted**, 없으면 모든 테스트입니다.

네이티브 테이블에서 정렬, 선택, 복사가 가능합니다. 데이터에 따라 **Test**, **Test file**(있는 경우), **Owner**(있는 경우), **Duration**(있는 경우), **Outcome** 열이 표시됩니다. 일치하는 결과를 최대 300행 표시하며, 결과가 모두 숨겨지면 **Show all tests** 링크로 모든 필터를 지울 수 있습니다.
...
로그 위에 **Summary** 탭이 계속 표시되므로 한 번의 클릭으로 개요로 돌아갈 수 있습니다. **Logs** 탭은 선택한 작업과 스크롤 위치를 유지합니다. <shortcut>L</shortcut>로도 Summary와 로그를 전환할 수 있습니다. 간결한 헤더에는 작업의 상태 아이콘, 이름, 시간 정보가 표시됩니다.
...
AI 로그 분석은 현재 작업 헤더나 단계의 **⋮** 메뉴에서 열어도 **파이프라인 실행 전체**를 분석합니다. 단계별·그룹별 분석은 추후 지원할 예정입니다. 각 단계 메뉴의 **링크 복사**와 **브라우저에서 열기**는 해당 단계를 가리킵니다. 연결된 동안 **Statistics** 옆 AI 버튼을 사용할 수 있습니다. **AI 분석**에서 **기존 실행 선택…**, **로그 분석**, **중지**, 기록 선택기를 사용합니다. 기록을 표시할 때는 AI를 호출하지 않으며 탭을 닫아도 분석은 계속됩니다. 기록은 PR 리뷰와 같은 보관 및 캐시 지우기 설정을 따릅니다. 새 파이프라인 실행은 시작하지 않습니다.

실행이 **완료**되면 작업 로그 헤더 막대의 오른쪽 끝에 **Analyze logs with AI** 버튼이 나타납니다. 클릭하면 Pipelines 도구 창에 **AI Analysis** 탭이 열리고,
구성된 AI 공급자가 실행 분석을 스트리밍합니다: 실패한 실행이라면 근본 원인, 로그의 근거, 수정 제안을, 성공한 실행이라면 무엇이 실행되었는지에 대한 짧은 요약을 보여 줍니다.

로그의 관련 부분만 전송됩니다 - 전체 로그 파일이 아니라 실패한 단계의 오류와 그 주변 문맥만 보냅니다. 결과는 실행별로 유지되므로 탭을 닫았다가 다시 열어도 새 AI 호출이 발생하지 않으며, 탭의 **Analyze Logs**
버튼으로 의도적으로 새 분석을 실행할 수 있습니다.

모든 실행은 이 탭을 공유합니다. 실행과 기록의 이전 분석을 선택할 수 있습니다. 선택 변경이나 보관된 기록을 다시 여는 동작은 AI를 호출하지 않습니다. 탭을 닫아도 분석은 계속되며 취소하려면 **중지**를 사용합니다. 기록은 PR 리뷰와 같은 보관 설정을 따릅니다.
...
줄에는 작업 로그와 동일한 의미의 색상이 적용되며 (오류 빨강, 경고 황색), 그룹을 접고 펼쳐도 줄 번호는 안정적으로 유지됩니다. **Summary** 탭으로 실행 개요로 돌아갑니다.
