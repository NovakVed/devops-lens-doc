...
### 리뷰 결과와 기록 {id="review-findings-and-history"}

**Run AI Review**를 실행하면 **Pull Requests** 도구 창의 **AI Reviews** 탭이 해당 PR 범위로 열립니다.
파일을 선택하면 다음 제안이 열리고, 파일을 펼치면 개별 제안을 선택할 수 있습니다. PR 검토와 같은 폴더 구조, 파일 아이콘, 변경 색상을 사용하며 아직 확인하지 않은 항목에는 파란 점이 표시됩니다.
제안, 파일 또는 폴더에 마우스를 올리면 체크박스로 확인함/확인하지 않음을 전환할 수 있습니다. 파일과 폴더는 하위 제안을 함께 전환하며, 일부만 확인한 경우 부분 체크를 표시합니다. Space 키와 우클릭 메뉴도 사용할 수 있습니다.
확인 상태는 실행별로 저장되며 제안을 닫거나 편집하는 것과 독립적입니다. 새 실행은 확인하지 않은 상태로 시작합니다. PR 또는 기록 선택기에 마우스를 올리면 검토한 파일 범위를 볼 수 있습니다. 최신 리뷰가 먼저 표시되며, 실행 기록이 두 개 이상이면 기록 선택기가 나타납니다.
**All pull requests**를 선택하면 현재 계정의 여러 저장소에 있는 리뷰를 볼 수 있습니다.

- **Run AI Review**는 PR의 최신 리비전을 가져온 후 시작합니다. **Stop Review**로 실행을 중지할 수 있습니다.
- 각 실행에는 공급자와 모델, 리비전, 완료 시간, 검토한 파일 수가 기록됩니다. 건너뛴 파일 수도 표시됩니다.
- **Dismiss Finding**과 **Restore Finding**은 diff의 제안에도 반영됩니다. 복원은 AI를 다시 호출하지 않습니다.
- **Opened for editing**은 일반 댓글 편집기로 전달되었다는 뜻입니다. 댓글이 저장되거나 게시되었다는 뜻은 아닙니다.
- 이전 결과는 검토 당시 리비전의 읽기 전용 diff에서 열립니다.

기록은 IDE 캐시에 로컬로 저장되어 재시작 후에도 유지됩니다. 캐시 크기 제한 내에서 PR당 최근 8회, 전체 80회를 보관합니다.
Azure DevOps에 업로드하거나 IDE 설정과 동기화하지 않습니다.

### AI 리뷰가 아직 최신인가요? {id="is-the-ai-review-still-current"}

타임라인의 **AI review** 섹션에는 상태와 **Open AI Reviews**, **Re-run**이 표시됩니다.
소스 또는 대상 리비전이 바뀌면 오래된 리뷰로 표시됩니다. **Re-run**은 새 유료 실행을 시작합니다.
기록을 열거나 숨긴 결과를 복원할 때는 AI를 호출하지 않습니다.

AI가 꺼져 있거나 설정되지 않은 경우 이 섹션은 숨겨집니다. 저장된 기록은 Pull Requests 도구 창 도구 모음의 **AI Reviews** 버튼에서 열 수 있습니다.
...
Pull Requests 도구 창의 **Statistics** 옆 AI 버튼으로 **AI Reviews**를 엽니다. 세부 정보를 불러오는 동안에도 **Run AI Review**와 **Open Pull Request**가 표시됩니다. **끌어오기 요청 선택…**에서 기록이 없는 PR도 검색하고 이 화면에서 리뷰를 실행할 수 있습니다. 아직 로드되지 않은 PR은 로드를 기다리며 실패하면 오류를 표시합니다.
...
PR 및 기존 실행 선택 화면에서 **Enter**는 강조된 항목을 AI 리뷰나 분석 대상으로 선택하고, **Shift+Enter**는 PR 타임라인이나 파이프라인 실행을 바로 엽니다. 기존 실행 선택 화면은 파이프라인 빠른 검색과 같은 간결한 행과 팝업 배경을 사용합니다.
